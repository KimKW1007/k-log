  ...
  return (
    <WithdrawForm onSubmit={handleSubmit(onSubmit)}>
      <WithdrawDescription>
        ...
        <p><span>탈퇴한 아이디는 재사용 및 복구가 불가하오니 신중하게 선택하시기 바랍니다.</span></p>
        <br />
        <p>탈퇴 후 회원정보 및 이용기록은 모두 삭제됩니다.</p>
        <p>다만, 탈퇴 후에도 게시물에 등록된 댓글은 그대로 남아 있습니다.</p>
        <p>삭제를 원하는 댓글이 있다면 <span>반드시 탈퇴 전 삭제하시기 바랍니다.</span></p>
      </WithdrawDescription>
      <WithdrawValidation>
        <p>회원 탈퇴를 하시려면 <i>"회원탈퇴"</i> 를 입력해주세요</p>
        <EditInput 
          type='text'
          register = {register('withdraw',{required : true, validate:{checkValue : (value)=> value === '회원탈퇴' || 'Error'}})}
          isError={Boolean(errors.withdraw?.message)}
        />
      </WithdrawValidation>
      <CheckAgreeBox>
